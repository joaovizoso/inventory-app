...
--
-- Seguro para correr mais do que uma vez (bucket com ON CONFLICT, políticas
-- com DROP IF EXISTS antes de recriar).

insert into storage.buckets (id, name, public)
values ('item-photos', 'item-photos', true)
on conflict (id) do nothing;

drop policy if exists "household members can upload item photos" on storage.objects;
...
drop policy if exists "household members can delete their item photos" on storage.objects;
